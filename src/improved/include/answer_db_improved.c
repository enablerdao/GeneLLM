...
void init_answer_db() {
    ...
    while (fgets(line, sizeof(line), fp) != NULL && answer_db_size < MAX_ANSWERS) {
        ...
        if (question != NULL && answer != NULL) {
            ...
            // 初期関連性スコアを設定
            answer_db[answer_db_size].relevance_score = 1.0;
...
const char* find_answer_with_score(const char* question, double* score) {
    ...
    for (i = 0; i < answer_db_size; i++) {
        if (strcasecmp(answer_db[i].question, question) == 0) {
    ...
    if (matched_question[0] != '\0') {
        for (int i = 0; i < answer_db_size; i++) {
            if (strcmp(answer_db[i].question, matched_question) == 0) {
                ...
                // スコアの範囲を0.5〜1.0に制限
                if (answer_db[i].relevance_score < 0.5) {
                    answer_db[i].relevance_score = 0.5;
                } else if (answer_db[i].relevance_score > 1.0) {
                    answer_db[i].relevance_score = 1.0;
                }
