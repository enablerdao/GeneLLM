...
// DNA形式から文を再構築
char* dna_decompress(DnaDictionary* dict, const char* dna_code) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 4) {
        ...
        // 再構築
        char* reconstructed = dna_decompress(dict, dna_code);
        printf("\n");
        printf("再構築された文: %s\n", reconstructed);
        
        // 辞書をファイルに保存
        if (dna_dictionary_save(dict, "dna_dictionary.txt")) {
            printf("\n");
            printf("辞書をファイルに保存しました: dna_dictionary.txt\n");
        }
    } else {
